...
class UserController extends Controller
{
   ...
   public function __construct()
    {   
        $this->middleware('auth')->except(['store','startSession','destroy','userManage', 'autocompleteLocation', 'loginWithWP', 'destroylaravel','heatAndFameScore']);
    }
    ...
    // to get the wp ulr from settings table
   public function getTheWpSiteUrl()
   {
        return $wp_url = SiteSettings::get()->toArray()[0]['wp_url'];
   }

  // to destroy the session of the logged in user from the wp site
   public function destroylaravel()
    {
        $wp_url = $this->getTheWpSiteUrl();
        $userID = request('userID');
        $session = Sessions::where('user_id', $userID)->delete();
        Auth::logout();
        return redirect($wp_url.'?action=logoutsotl');
    }
   ...
   public function loginWithWP(Request $request)
        ...
        if(empty($check_temp)){
         ...
         $username = request('userfullname');
            ...
            if($temp){
             ...
             \DB::table('temps')->where('user_id',request('user_id'))->update(array('rand_token' => request('rand_token')));
             \DB::table('users')->where('id',request('user_id'))->update(array('role' => request('userrole')));
    ...
    $totalFame = 0;
        if($count || $totalHelpFul){
            $totalFame = $count * 2 + $totalHelpFul * 5;
        }
        return array('totalReview' => $count, 'totalHelpful' => $totalHelpFul, 'totalFame' => $totalFame);
    }
    //get heat and fame score for wp user's
    public function heatAndFameScore($id)
    {
       $count = Reviews::where('user_id','=',$id)->count();
        $reviews = Reviews::where('user_id','=',$id)->get()->toArray();
        //$mosthelpfulreview = Reviews::select('description')->where('user_id','=',$id)->orderBy('helpfullreviewcount','DESC')->take(1)->get()->toArray();
        $mosthelpfulreview = Reviews::select('description','user_id','created_at')->orderBy('helpfullreviewcount','DESC')->take(1)->get()->toArray();
        $mostreviewUser = User::select('name')->where('id','=', $mosthelpfulreview[0]['user_id'])->get()->toArray();
        $mostreviewUserName = $mostreviewUser[0]['name'];
        $totalHelpFul = 0; 
        if($reviews){
            foreach ($reviews as $review) {
                $totalHelpFul = $totalHelpFul + $review['helpfullreviewcount'];
    ...
    $totalFame = 0;
        ...
        return array('totalReview' => $count, 'totalHelpful' => $totalHelpFul, 'totalFame' => $totalFame, 'mosthelpfulreview' => $mosthelpfulreview, 'username' => $mostreviewUserName);
    }
